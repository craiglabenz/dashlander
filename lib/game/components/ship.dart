...
class ShipComponent extends PositionComponent {
  ...
  ShipComponent({required this.state, this.isGhost = false, this.tintColor}) : super(anchor: Anchor.center, size: Vector2(36, 36)) {
    final opacity = isGhost ? 0.4 : 1.0; // Slightly higher opacity for visibility
  ...
  void render(Canvas canvas) {
    ...
    if (!isGhost) {
      canvas.saveLayer(null, Paint()
        ..imageFilter = const ColorFilter.mode(Color(0x80FF00FF), BlendMode.srcATop)
