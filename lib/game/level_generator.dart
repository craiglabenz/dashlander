...
class LevelGenerator {
  static LevelData generate({
    ...
    final double maxHeight = maxTerrainHeight ?? PhysicsConstants.maxTerrainHeight;
    ...
    for (int i = 0; i < segments; i++) {
      ...
      double h = sin(t * freq) * maxHeight * 0.5 + cos(t * freq * 2.3) * maxHeight * 0.25;
      // Add chaos noise
    ...
    // Flatten the pads
    for (int p in padIndices) {
      double padHeight = heights[p];
      for (int i = 1; i <= padWidth; i++) {
        heights[(p + i) % segments] = padHeight;
      }
    }
