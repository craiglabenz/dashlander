...
class _MinimapState extends State<Minimap> {
  ...
  void didUpdateWidget(Minimap oldWidget) {
    ...
    if (oldWidget.telemetry.x != widget.telemetry.x || oldWidget.telemetry.y != widget.telemetry.y) {
...
class _MinimapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double maxDrawRadius = PhysicsConstants.moonRadius + PhysicsConstants.maxTerrainHeight + 1500;
    ...
    final Paint moonCorePaint = Paint()
      ..color = Colors.grey.shade900
      ..style = PaintingStyle.fill;
    
    canvas.drawCircle(center, PhysicsConstants.moonRadius * scale, moonCorePaint);

    // Draw the terrain topology
    final Paint terrainOutlinePaint = Paint()
    ...
    if (path.length > 1) {
      ...
      final Offset firstPos = center + Offset(path.first.dx * scale, path.first.dy * scale);
      drawPath.moveTo(firstPos.dx, firstPos.dy);
      
      for (int i = 1; i < path.length; i++) {
        final Offset p = center + Offset(path[i].dx * scale, path[i].dy * scale);
        drawPath.lineTo(p.dx, p.dy);
      }
      
      // Draw line to current exact position
      final Offset currentPos = center + Offset(telemetry.x * scale, telemetry.y * scale);
    ...
    final Offset shipOffset = center + Offset(telemetry.x * scale, telemetry.y * scale);

    canvas.drawCircle(shipOffset, 2.0, shipPaint); // 2.0 radius makes it a tiny dot
  }

  @override
  bool shouldRepaint(covariant _MinimapPainter oldDelegate) {
    return oldDelegate.telemetry.x != telemetry.x || oldDelegate.telemetry.y != telemetry.y;
  }
}
